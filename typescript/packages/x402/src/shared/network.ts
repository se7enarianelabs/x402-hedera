import { EvmNetworkToChainId, HederaNetworkToChainId, Network, SvmNetworkToChainId } from "../types/shared";
...
export function getNetworkId(network: Network): number {
  ...
  if (HederaNetworkToChainId.has(network)) {
    return HederaNetworkToChainId.get(network)!;
  }
